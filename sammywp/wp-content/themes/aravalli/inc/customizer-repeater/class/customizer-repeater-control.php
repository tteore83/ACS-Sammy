...
class ARAVALLI_Repeater extends WP_Customize_Control {
	...
	public function __construct( $manager, $id, $args = array() ) {
		...
		$this->add_field_label = esc_html__( 'Add new field', 'aravalli' );
		if ( ! empty( $args['add_field_label'] ) ) {
			$this->add_field_label = $args['add_field_label'];
		}

		$this->boxtitle = esc_html__( 'Customizer Repeater', 'aravalli' );
	...
	public function enqueue() {
		...
		wp_enqueue_style( 'aravalli_customizer-repeater-admin-stylesheet', get_template_directory_uri() . '/inc/customizer-repeater/css/admin-style.css', array(), 999 );

		wp_enqueue_style( 'wp-color-picker' );

		wp_enqueue_script( 'aravalli_customizer-repeater-script', get_template_directory_uri() . '/inc/customizer-repeater/js/customizer_repeater.js', array('jquery', 'jquery-ui-draggable', 'wp-color-picker' ), 999, true  );

		wp_enqueue_script( 'aravalli_customizer-repeater-fontawesome-iconpicker', get_template_directory_uri() . '/inc/customizer-repeater/js/fontawesome-iconpicker.js', array( 'jquery' ), 999, true );

		wp_enqueue_style( 'aravalli_customizer-repeater-fontawesome-iconpicker-script', get_template_directory_uri() . '/inc/customizer-repeater/css/fontawesome-iconpicker.min.css', array(), 999 );
	}
					...
					<div class="customizer-repeater-box-content-hidden">
						...
						if($this->customizer_repeater_title_control==true){
							$this->input_control(array(
								'label' => apply_filters('aravalli_repeater_input_labels_filter', esc_html__( 'Title','aravalli' ), $this->id, 'customizer_repeater_title_control' ),
						...
						if($this->customizer_repeater_subtitle_control==true){
							$this->input_control(array(
								'label' => apply_filters('aravalli_repeater_input_labels_filter', esc_html__( 'Subtitle','aravalli' ), $this->id, 'customizer_repeater_subtitle_control' ),
						...
						if($this->customizer_repeater_text_control==true){
							$this->input_control(array(
								'label' => apply_filters('aravalli_repeater_input_labels_filter', esc_html__( 'Description','aravalli' ), $this->id, 'customizer_repeater_text_control' ),
						...
						if($this->customizer_repeater_text2_control==true){
                            $this->input_control(array(
                                'label' => apply_filters('aravalli_repeater_input_labels_filter', esc_html__( 'Button Label','aravalli' ), $this->id, 'customizer_repeater_text2_control' ),
						...
						if($this->customizer_repeater_link2_control){
                            $this->input_control(array(
                                'label' => apply_filters('aravalli_repeater_input_labels_filter', esc_html__( 'skills','aravalli' ), $this->id, 'customizer_repeater_link2_control' ),
						...
						if($this->customizer_repeater_button_text_control){
							...
							'aravalli'), $this->id, 'customizer_repeater_button_text_control'),
						...
						if($this->customizer_repeater_link_control){
							$this->input_control(array(
								'label' => apply_filters('aravalli_repeater_input_labels_filter', esc_html__( 'Link','aravalli' ), $this->id, 'customizer_repeater_link_control' ),
						...
						if($this->customizer_repeater_video_url_control){
							...
							'aravalli'), $this->id, 'customizer_repeater_video_url_control'),
						...
						if($this->customizer_repeater_color_control == true){
							$this->input_control(array(
								'label' => apply_filters('aravalli_repeater_input_labels_filter', esc_html__( 'Color','aravalli' ), $this->id, 'customizer_repeater_color_control' ),
						...
						if($this->customizer_repeater_shortcode_control==true){
							$this->input_control(array(
								'label' => apply_filters('aravalli_repeater_input_labels_filter', esc_html__( 'Shortcode','aravalli' ), $this->id, 'customizer_repeater_shortcode_control' ),
						...
						if($this->customizer_repeater_designation_control==true){
							$this->input_control(array(
								'label' => apply_filters('aravalli_repeater_input_labels_filter', esc_html__( 'Designation','aravalli' ), $this->id, 'customizer_repeater_designation_control' ),
						...
						<button type="button" class="social-repeater-general-control-remove-field" <?php if ( $it == 0 ) {
							...
							<?php printf( __( 'Delete %s', 'aravalli' ), $this->boxtitle );	?>
						</button>
				...
				<div class="customizer-repeater-box-content-hidden">
					...
					if ( $this->customizer_repeater_text2_control == true ) {
					 ...
					 if ( $this->customizer_repeater_link2_control == true ) {
                        $this->input_control( array(
                            'label' => apply_filters('aravalli_repeater_input_labels_filter', esc_html__( 'skill','aravalli' ), $this->id, 'customizer_repeater_link2_control' ),
					...
					<button type="button" class="social-repeater-general-control-remove-field button" style="display:none;">
						<?php esc_html_e( 'Delete field', 'aravalli' ); ?>
					</button>
				</div>
			</div>
			<?php
...
//print_r($options);
	...
	<?php esc_html_e('Open link in new tab:','aravalli'); ?>
	<span class="switch">
	...
	private function icon_picker_control($value = '', $show = '', $class=''){ ?>
		<div class="social-repeater-general-control-icon" <?php if( $show === 'customizer_repeater_image' || $show === 'customizer_repeater_none' ) { echo esc_attr('style="display:none;"'); } ?>>
            <span class="customize-control-title">
                <?php esc_html_e('Icon','aravalli'); ?>
            </span>
			<span class="description customize-control-description">
                <?php
                echo sprintf(
	                esc_html__( 'Note: Some icons may not be displayed here. You can see the full list of icons at %1$s.', 'aravalli' ),
	                sprintf( '<a href="http://fontawesome.io/icons/" rel="nofollow">%s</a>', esc_html__( 'http://fontawesome.io/icons/', 'aravalli' ) )
                ); ?>
            </span>
		...
		<div class="customizer-repeater-image-control" <?php if( $show === 'customizer_repeater_icon' || $show === 'customizer_repeater_none' ) { echo esc_attr('style="display:none;"'); } ?>>
			...
			<input type="button" class="button button-secondary customizer-repeater-custom-media-button <?php if($class="aravalli_overlimit") { echo esc_attr('aravalli-uploading-img-btn');}?> <?php echo esc_attr($auto);?>" value="<?php esc_attr_e( 'Upload Image','aravalli' ); ?>" />
		</div>
		<?php
	}
	...
	private function image_control2($value = '', $show = '', $class='', $auto='', $sections=''){ 
		...
		<div class="customizer-repeater-image2-control">
            <span class="customize-control-title">
                <?php esc_html_e('Slider Image','aravalli')?>
            </span>
	...
	private function icon_type_choice($value='customizer_repeater_icon', $aravalli_limit=''){ ?>
		<span class="customize-control-title">
            <?php esc_html_e('Image type','aravalli');?>
        </span>
		<select class="customizer-repeater-image-choice  <?php echo esc_attr($aravalli_limit);?>">
			<option value="customizer_repeater_icon" <?php selected($value,'customizer_repeater_icon');?>><?php esc_html_e('Icon','aravalli'); ?></option>
			<option value="customizer_repeater_image" <?php selected($value,'customizer_repeater_image');?>><?php esc_html_e('Image','aravalli'); ?></option>
			<option value="customizer_repeater_none" <?php selected($value,'customizer_repeater_none');?>><?php esc_html_e('None','aravalli'); ?></option>
		</select>
		<?php
	}
	...
	private function repeater_control($value = '', $aravalli_limit='', $aravalli_type_with_id=''){
		...
		$show_del        = 0; ?>
		<span class="customize-control-title"><?php esc_html_e( 'Social icons', 'aravalli' ); ?></span>
		<?php
		if(!empty($value)) {
		...
		if ( ( count( $social_repeater ) == 1 && '' === $social_repeater[0] ) || empty( $social_repeater ) ) { ?>
			<div class="customizer-repeater-social-repeater">
				<div class="customizer-repeater-social-repeater-container">
					...
					<input type="text" class="customizer-repeater-social-repeater-link team_linkdata_<?php echo $aravalli_limit;?> <?php echo esc_attr($aravalli_type_with_id);?>"
					       placeholder="<?php esc_attr_e( 'Link', 'aravalli' ); ?>">
					<input type="hidden" class="customizer-repeater-social-repeater-id" value="">
					<button class="social-repeater-remove-social-item" style="display:none">
						<?php esc_html_e( 'Remove Icon', 'aravalli' ); ?>
					</button>
				</div>
				<input type="hidden" id="social-repeater-socials-repeater-colector" class="social-repeater-socials-repeater-colector" value=""/>
			</div>
			<button class="social-repeater-add-social-item button-secondary "><?php esc_html_e( 'Add Icon', 'aravalli' ); ?></button>
			<?php
		} else { ?>
			<div class="customizer-repeater-social-repeater">
				<?php
				foreach ( $social_repeater as $social_icon ) {
					...
					<div class="customizer-repeater-social-repeater-container">
						...
						<button class="social-repeater-remove-social-item"
						        ...
						        } ?>"><?php esc_html_e( 'Remove Icon', 'aravalli' ); ?></button>
					</div>
					<?php
				} ?>
			...
			<button class="social-repeater-add-social-item button-secondary <?php echo esc_attr($aravalli_limit);?> <?php echo esc_attr($aravalli_type_with_id);?>"><?php esc_html_e( 'Add Icon', 'aravalli' ); ?></button>
			<?php
		}
	}
}
...
/**
  ...
  * @return string
  */
 function aravalli_repeater_labels( $string, $id, $control ) {
	if ( $id === 'room_contents' ) {
		if ( $control === 'customizer_repeater_button_text_control' ) {
			return esc_html__( 'Ribbon Text','aravalli' );
		}
		if ( $control === 'customizer_repeater_title_control' ) {
			return esc_html__( 'Price Badge','aravalli' );
		}
		if ( $control === 'customizer_repeater_subtitle_control' ) {
			return esc_html__( 'Title','aravalli' );
		}
		if ( $control === 'customizer_repeater_text_control' ) {
			return esc_html__( 'Description','aravalli' );
		}
	}
	
	return $string;
 }
 add_filter( 'aravalli_repeater_input_labels_filter','aravalli_repeater_labels', 10 , 3 );